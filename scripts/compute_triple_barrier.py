...
def get_triple_barrier_labels(
    ...
    # Floor: minimum barrier width to avoid noise labels
    pt_threshold = np.maximum(pt_threshold, min_ret)
    sl_threshold = np.maximum(sl_threshold, min_ret)
...
def main():
    ...
    ap.add_argument("--pt", type=float, default=1.5,
                    help="Profit Take multiplier × daily_vol (was 2.0 × micro_vol)")
    ap.add_argument("--sl", type=float, default=1.5,
                    help="Stop Loss multiplier × daily_vol (was 2.0 × micro_vol)")
    ap.add_argument("--v-bars", type=int, default=216,
                    help="Vertical Barrier in dollar bars (216 ≈ 9h)")
    ap.add_argument("--min-ret", type=float, default=0.003,
                    help="Minimum barrier width as fraction of close (0.3%)")
